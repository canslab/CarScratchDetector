...
{
	for (const auto& eachPoint : in_contour)
	{
		if (!IsThisPointInROI(in_ROI, eachPoint))
		{
			return false;
		}
	}
...
void ComputePCA_VisualizeItsResult(const cv::Mat& img, const std::vector<cv::Point2f>& in_pts, double &out_largeEigValue, double &out_smallEigValue, double &out_degree, cv::Mat& out_drawnImage)
{
	...
	out_degree = std::atan2(eigen_vecs[0].y, eigen_vecs[0].x) * 180 / 3.141592;
}
...
{
	cv::Mat luvOriginalImageMat, filteredImageMat_luv, filteredImageInHSV, labelMap, edgeGradientMap;

	const int kTotalPixels = in_srcImage.total();								// 총 픽셀수 저장
	...
	const int kNumOfTopClusters = 5;

	cv::Mat hsvPlanes[3];
	cv::Mat LUVPlanes[3];
	std::vector<int> hueArray(kHueIntervals);
	std::vector<int> satArray(kSatIntervals);
	std::unordered_map<int, MeanShiftCluster> clusters; 						// Cluster 모음
	std::vector<MeanShiftCluster> topNClusters;									// 상위 N개 클러스터 모음

																				// 원본 이미지를 이용, Mean Shift CLustering 수행
	cv::cvtColor(in_srcImage, luvOriginalImageMat, CV_BGR2Luv);
	...
	GetTopNClusters(clusters, kNumOfTopClusters, topNClusters);		// 규모가 큰 top (kN)개 클러스터 얻어옴
	const cv::Point3i seedClusterColor = topNClusters[0].GetHSVColor();

	// Mean Shift Filtering 된 HSV 이미지가 필요해서 만듬
	cv::cvtColor(filteredImageMat_luv, filteredImageMat_luv, CV_Luv2BGR);
	cv::cvtColor(filteredImageMat_luv, filteredImageInHSV, CV_BGR2HSV);

	// 필터링 된 이미지를 HSV로 각 채널별로 분리
	cv::split(filteredImageInHSV, hsvPlanes);

	uchar biggestClusterHValue = seedClusterColor.x;
	uchar biggestClusterVValue = seedClusterColor.z;

	// 이미지의 색상 분포를 파악하는데 사용, Hue분포, Saturation 분포 계산
	...
	cv::Mat lastBinaryImage(in_srcImage.rows, in_srcImage.cols, CV_8UC1, cv::Scalar::all(0));
	...
	if (bImageHasLowSaturationCarColor)
	{
		...
		{
			upperValueBound = ((int)biggestClusterVValue + 60 <= 255) ? (biggestClusterVValue + 60) : 255;
		}
		lowerValueBound = 80;

		// 채도값으로 필터링하고 명도값으로 필터링 해서 세그멘트한다.
...
void DetectScratchPointsInsideCarBody(const cv::Mat in_targetImage, const cv::Rect in_ROI, const cv::Mat in_carBodySegmentedImage, const std::vector<cv::Point> in_carBodyContourPoints,
	std::vector<cv::Point2f> &out_scratchPoints)
{
	...
	if (in_carBodySegmentedImage.data)
	{
		...
		auto marginWidth_ROI = in_ROI.width / 5;
		auto marginHeight_ROI = in_ROI.height / 5;
		...
		for (auto& eachPoint : scratchCandidates)
		{
			...
			{
				int count = 0;
				for (auto rowIndex = eachPoint.y - 4; rowIndex < eachPoint.y + 4; ++rowIndex)
				{
					for (auto colIndex = eachPoint.x - 4; colIndex < eachPoint.x + 4; ++colIndex)
					{
...
void ExtractImageDescriptorFromMat(const cv::Mat& in_targetImage, ImageDescriptor& out_analyzeResult, bool in_bShowExperimentResult)
{
	...
	{
		// ROI 만들기
		const int kROIParameter_Dividier = 12;										// ROI를 만들 때, Width, Height를 각각 몇 등분할지 나타냄. 
		...
		// 차체 세그멘테이션
		ExtractCarBody(in_targetImage, kROI, carBodyBinaryImage, carBodyContourPoints);

		// 검출된 차체 영역에서 스크래치 검출 수행 
		DetectScratchPointsInsideCarBody(in_targetImage, kROI, carBodyBinaryImage, carBodyContourPoints, scratchPoints);
		...
		// 밀도 기반 그룹핑 수행
		GroupClusters(scratchPoints, scratchClusters);
		...
		if (scratchClusters.size() != 0)
		{
			...
			double largeEigValue = 0, smallEigValue = 0, eigValueRatio = 0, degree = 0;
			ComputePCA_VisualizeItsResult(in_targetImage, largestCluster, largeEigValue, smallEigValue, degree, largestClusterImage);
